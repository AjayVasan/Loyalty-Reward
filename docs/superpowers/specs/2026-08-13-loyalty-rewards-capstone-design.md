...
**Transaction** — as given, field names unchanged: `txnID` (UUID, key), `customerID` (association to Customer), `channel` (String — "Online"/"Store"), `amount` (Decimal(10,2)), `txnDate` (DateTime), `pointsEarned` (Integer, computed by handler).

**Redemption** — as given, field names unchanged: `redeemID` (UUID, key), `customerID` (association to Customer), `pointsUsed` (Integer), `redeemDate` (DateTime), `remarks` (String).

*Note on the `customerID` association name*: kept exactly as the PDF's Domain Modelling table names it — table-level field names are followed strictly, no renaming. Consequence accepted deliberately: since Customer's key is itself `customerID`, CAP's `<association-name>_<target-key-name>` convention generates the physical/OData column `customerID_customerID`. This is unusual-looking but functionally correct; handler code and UI annotations below use that exact generated name.
